...
void BenchmarkRunner::_benchmark_permuted_query_set() {
  ...
  while (state.keep_running() && finished_query_set_runs.load(std::memory_order_relaxed) < _config.max_num_query_runs) {
    ...
    if (currently_running_clients.load(std::memory_order_relaxed) < _config.clients) {
      ...
      for (const auto& query_id : query_ids) {
        auto pipeline_task = _create_pipeline_execution_task(query_id);

        // The on_query_done callback will be executed after the query has been executed in order to
        ...
        auto on_query_done = [pipeline_task, query_id, number_of_queries, query_run_begin, &currently_running_clients,
  ...
  for (const auto& query_id : _query_generator->selected_queries()) {
    ...
    while (state.keep_running() && result.num_iterations.load(std::memory_order_relaxed) < _config.max_num_query_runs) {
      ...
      if (currently_running_clients.load(std::memory_order_relaxed) < _config.clients) {
        ...
        // The on_query_done callback will be executed after the query has been executed in order to
        // to measure its duration as well as signal that the query was finished
        auto on_query_done = [pipeline_task, &currently_running_clients, &result, &state]() {
          currently_running_clients--;
          if (!state.is_done()) {  // To prevent queries to add their results after the time is up
            result.num_iterations++;
            result.metrics.push_back(pipeline_task->get_sql_pipeline()->metrics());
...
std::shared_ptr<PipelineExecutionTask> BenchmarkRunner::_create_pipeline_execution_task(const QueryID query_id) const {
  // Setup SQLPipelineBuilder
  ...
  return std::make_shared<PipelineExecutionTask>(pipeline_builder);
}
